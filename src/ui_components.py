...
def render_papers_section(papers: list, top_n: int = 5):
    ...
    if len(papers) > top_n:
        ...
        if st.checkbox(f"📚 Show {len(remaining)} more papers", key="show_more_papers"):
...
def render_books_section(books: list, top_n: int = 5):
    ...
    if len(books) > top_n:
        if st.checkbox(f"📚 Show {len(books) - top_n} more books", key="show_more_books"):
...
def render_repos_section(repos: list, top_n: int = 5):
    ...
    if len(repos) > top_n:
        if st.checkbox(f"📂 Show {len(repos) - top_n} more repositories", key="show_more_repos"):
...
def render_websites_section(websites: list, top_n: int = 8):
    ...
    if len(websites) > top_n:
        if st.checkbox(f"🌐 Show {len(websites) - top_n} more websites", key="show_more_websites"):
            for resource in websites[top_n:]:
                render_website_card(resource)
...
def render_videos_section(videos: list, top_n: int = 5):
    ...
    if len(videos) > top_n:
        if st.checkbox(f"📺 Show {len(videos) - top_n} more videos", key="show_more_videos"):
...
def render_retrieved_context(results: list):
    ...
    for i, r in enumerate(results[:6]):
        ...
        with st.container(border=True):
            st.markdown(f"**{icon} [{mod.upper()}] Result #{i+1}** (score: {score:.3f})")
