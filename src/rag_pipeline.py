...
def node_parse(state: RAGState) -> dict:
...
def node_index_text(state: RAGState) -> dict:
...
def node_caption_images(state: RAGState) -> dict:
    ...
    try:
        ...
        return {"image_data": image_data, "image_count": count}
    except Exception as e:
        print(f"[rag_pipeline] Image captioning failed: {e}")
        return {"image_count": 0}


def node_process_tables(state: RAGState) -> dict:
...
def index_document(
    ...
    try:
        ...
        state.update(node_index_text(state))
        state.update(node_caption_images(state))
        state.update(node_process_tables(state))
